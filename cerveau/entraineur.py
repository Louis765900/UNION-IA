...
class Entraineur:
    ...
    def charger_intents(self):
        self.intents = []
        index_tags = {}  # tag → index dans self.intents (pour fusion)
        for chemin in self._chemins_intents():
            with open(chemin, "r", encoding="utf-8") as f:
                for intent in json.load(f)["intents"]:
                    tag = intent["tag"]
                    if tag in index_tags:
                        # Fusionner les patterns (dédupliqués) sans toucher aux réponses
    ...
    def entrainer(self, force=False):
        ...
        self.reseau.entrainer(X, y, epochs=5000, learning_rate=0.05)
